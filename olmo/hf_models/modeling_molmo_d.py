...
@dataclass
class MolmoDCausalLMOutputWithPast(CausalLMOutputWithPast):
    aux_loss: Optional[tuple] = None

@dataclass
class MolmoDConfig(Qwen2Config):
    model_type = "molmo_d"
...
class MolmoDModel(Qwen2Model):
    def __init__(self, config: MolmoDConfig):
    ...
    def init_vision_backbone(self, config: MolmoDConfig):
        if config.vision_backbone is not None:
            self.vision_backbone = MolmoVisionBackbone.build(config.vision_backbone)
...
class MolmoDForCausalLM(Qwen2ForCausalLM):
    config_class = MolmoDConfig

    def __init__(
        self, 
        config: MolmoDConfig
    ):
        super(MolmoDForCausalLM, self).__init__(config)

        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.model = MolmoDModel(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        kwargs.pop("pad_attention_mask", None)
        ...
        return MolmoDCausalLMOutputWithPast(
            loss=loss,
...
AutoConfig.register("molmo_d", MolmoDConfig)
AutoModelForCausalLM.register(MolmoDConfig, MolmoDForCausalLM)