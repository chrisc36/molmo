...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        import copy
        self._eval_tokenizer = copy.deepcopy(self.tokenizer)
        self._eval_tokenizer.data_formatter_cfg["is_training"] = False
    ...
    def inference_eval(self):
        ...
        for evaluator in self.evaluators:
            ...
            print(f"Running evaluation for '{evaluator.label}'...")
            dataset_metrics = evaluator.run_hf(
                self.model,
            ...
            logger.info(f"Eval for '{evaluator.label}' done in {time.perf_counter()-t0:0.1f} seconds")
        self.model.train()
        return all_metrics

    def _inner_training_loop(
        self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None, evaluators=None,
    ):
        ...
        logger.info(f"Total number of parameters: {sum(p.numel() for p in self.model.parameters()):,d}")

        len_dataloader = None
        num_train_tokens = None
        if has_length(train_dataloader):
            len_dataloader = len(train_dataloader)
            num_update_steps_per_epoch = len_dataloader // (args.gradient_accumulation_steps * args.world_size)
        ...
        print(f"  Number of parameters = {get_model_param_count(model, trainable_only=False):,}")
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if ((self.state.global_step > 0 and self.state.global_step != infer_step and self.state.global_step % max(self.args.save_steps, 1000) == 0) or self.state.global_step == max_steps):
                    if not self.is_world_process_zero():
                        try:
                            eval_metrics = self.inference_eval()
                        except Exception as e:
                            logger.error(f"Error in inference evaluation on GPU {self.args.local_rank}: {e}")
                            eval_metrics = {}
                    else:
                        eval_metrics = self.inference_eval()
                    infer_step = self.state.global_step
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        ...
        if os.path.isfile(weights_file) or os.path.isfile(safe_weights_file) or is_fsdp_ckpt:
            weights_only_kwarg = {"weights_only": False} if is_torch_greater_or_equal_than_1_13 else {} #TODO: should be true?
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        if checkpoint_file_exists:
            ...
            # Load scheduler(s)
            sched_path = os.path.join(checkpoint, SCHEDULER_NAME)
            if os.path.exists(sched_path):
                with warnings.catch_warnings(record=True) as caught_warnings:
                    self.lr_scheduler.load_state_dict(torch.load(sched_path))
                reissue_pt_warnings(caught_warnings)
